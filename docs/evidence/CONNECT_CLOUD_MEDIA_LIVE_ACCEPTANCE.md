...
| Live broadcast to a real destination | Core `request_live_broadcast`; AWS IVS channel (owner-chosen 2026-09-18, replaces Cloudflare Stream) with a per-session stream key; **an external fan-out component, because IVS has no documented native restream to YouTube/Facebook/Twitch**; provider OAuth linking or a user-entered RTMP key held server-side | BIN-39, BIN-46 | AWS account/spend approval; IVS ingest test over SRT; fan-out spike (Amendment A1 gap 1) | A real test stream reaches IVS and one external destination through a single boat uplink; disconnect/reconnect measured. Needs your approval of destination and content |
...
AWS-IVS-live / Cloudflare-R2-durable-media direction (Amendment A1, replacing
the Cloudflare-Stream / AWS-S3 direction of architecture v0.1) is recorded as
