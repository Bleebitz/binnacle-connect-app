...
|---|---|---|---|---|
| Real media upload (queue to confirmed remote) | Deployed private Cloudflare R2 bucket + presigned-upload contract + identity for who may upload. Direction changed 2026-09-18 by the owner from AWS S3 to Cloudflare R2 (`docs/architecture/CONNECT_CLOUD_ARCHITECTURE_AMENDMENT_A1.md`); nothing is deployed and no contract is final | BIN-48, BIN-41, BIN-46 | Cloudflare account/spend approval, then a `MediaUploadService` that uploads with a single-object presigned R2 URL | Upload the test clip, compare checksum, mark uploaded only on match; unauthenticated GET is refused |
| Authorized playback / share | R2 presigned URLs or a Worker that checks the Binnacle token and the rider's pointer (replaces CloudFront signed URLs); custom-domain presigned support is unverified | BIN-48, BIN-46 | Prototype against a real R2 object | Authorized URL plays; expired/unauthorized URL is blocked |
| Live broadcast to a real destination | Core `request_live_broadcast`; a Cloudflare Stream live input per session (owner-chosen 2026-09-18; **AWS IVS was dropped**), with fan-out via Stream simulcast outputs; provider OAuth linking or a user-entered RTMP key held server-side | BIN-39, BIN-46 | Cloudflare credentials from the owner's secrets manager; run the approved $50 spike (`docs/architecture/BIN-39_FANOUT_SPIKE.md`) | A real test stream reaches Stream playback and 2+ private destinations through a single boat uplink; per-destination state independent; measured cost within 20% of the tier's revenue. Needs your approval of destination and content for any real broadcast |
| Shared Crew / privacy / reactions across devices | Global Rider Accounts (OAuth), Vision-unit entity, ephemeral session tokens, offline handshake — designed in `CORE_USER_ACCOUNT_API_PROPOSAL.md` v2.3 (Decoupled Identity Architecture, Supabase Auth); not built; owner decisions in its section 11 | BIN-46, BIN-40 | Phase 1 cloud identity (Supabase Auth + token service) | A rider from a different account joins a boat as crew with cellular disabled; a second device sees / does not see content per permission |
...
Cloudflare-Stream-live / Cloudflare-R2-durable-media direction (Amendment A1 rev 3;
the AWS S3, CloudFront, MediaConvert and IVS directions were dropped) is recorded as
*chosen providers, not deployed, not contracted*.
